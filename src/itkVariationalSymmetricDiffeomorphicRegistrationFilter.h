 ...
 * differential operator associated with the regularization term \f$ S \f$, VariationalSymmetricDiffeomorphicRegistrationFilter
 ...
 *   - <b>do</b>
 *     - compute the forward update field \f$ f_k^{forw} \f$ using \f$ R(x) \f$ and the warped image \f$ T\circ\phi_k \f$
 *     - compute the backward update field \f$ f_k^{back} \f$ using the warped image \f$ R(x)\circ\phi_k^{-1} \f$ and \f$ T \f$
 *     - compute the next velocity field by \f$ v_{k+1} = (Id - \tau\alpha A)^{-1}(v_k + \frac{\tau}{2}(f_k^{forw} + f_k^{back}))\f$
 *     - compute the next transformation \f$ \phi_{k+1}=exp(v_{k+1})\f$ and the inverse \f$ \phi_{k+1}^{-1}=exp(-v_{k+1})\f$
